...
class SettingsView(ttk.Frame):
    ...
    def upload_map(self):
        ...
        x_max = self.app.state.map_x_max_var.get()
        y_max = self.app.state.map_y_max_var.get()
        
        success, map_filename = self.app.config_manager.add_new_map(file_path, x_max, y_max)
        
        if success:
            self.refresh_map_list()
            self.app.state.selected_map_var.set(map_filename)
    ...
    def prompt_for_admin_password(self, event=None):
        ...
        if password == "admin":
            self.app.state.admin_mode_enabled.set(True)
            ...
            self.app.state.admin_mode_enabled.set(False)
